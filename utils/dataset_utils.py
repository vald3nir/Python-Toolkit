...
from toolkit.utils.date_utils import LOCAL_TIME_ZONE, DATE_FORMAT_UTC
...
def format_time_zone(frame: pd.Series) -> pd.Series:
    _frame = frame.map(lambda x: x.tz_convert(LOCAL_TIME_ZONE)).dt.strftime(DATE_FORMAT_UTC)
